...
export async function seedData() {
  ...
  _seedPromise = (async () => {
    ...
    const movies = [
      {
        ...
        description: 'Cuộc chiến quyền lực và lời tiên tri trên hành tinh cát — trở lại hoành tráng.',
      },
      {
        title: 'Inside Out 2',
        genre: 'Animation',
    ...
    await db.withTransactionAsync(async () => {
      await db.runAsync("INSERT INTO Users (username, password, fullName) VALUES ('admin', '123456', 'Quản trị rạp')");
      for (const movie of movies) {
        await db.runAsync(
          'INSERT INTO Movies (title, genre, duration, rating, posterUrl, description, basePrice) VALUES (?, ?, ?, ?, ?, ?, ?)',
          [movie.title, movie.genre, movie.duration, movie.rating, movie.posterUrl, movie.description, movie.basePrice]
        );
      }
