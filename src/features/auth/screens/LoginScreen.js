...
import { View, Text, TextInput, TouchableOpacity, StyleSheet, Animated, Easing, KeyboardAvoidingView, Platform, Dimensions } from 'react-native';
        ...
        <Animated.View style={[styles.formCard, { transform: [{ translateX: shakeAnim }] }]}>
          ...
          {!isLoginSync && (
            <View style={styles.inputGroup}>
              <Text style={styles.label}>Tên của bạn là gì nè?</Text>
              <TextInput 
                style={styles.input} 
                value={fullName} onChangeText={setFullName}
                placeholder="Ví dụ: Công Chúa Nhỏ"
          ...
          <View style={styles.switchWrap}>
            <Text style={styles.switchText}>
              {isLoginSync ? 'Bạn chưa có tài khoản?' : 'Đã có tài khoản rồi?'}
            </Text>
            <TouchableOpacity onPress={() => { setIsLoginSync(!isLoginSync); setErrorMsg(''); }}>
...
const styles = StyleSheet.create({
  ...
  logo: { fontSize: 42, fontWeight: '900', color: COLORS.surface, marginBottom: 10, textShadowColor: 'rgba(0,0,0,0.1)', textShadowOffset: {width: 0, height: 4}, textShadowRadius: 10 },
  slogan: { fontSize: FONTS.sizes.md, color: COLORS.surface, fontWeight: '600' },
  bottomSection: { height: height * 0.65, backgroundColor: COLORS.background, borderTopLeftRadius: 40, borderTopRightRadius: 40, padding: 25 },
  formCard: { 
    backgroundColor: COLORS.surface, borderRadius: 32, padding: 30, marginTop: -60,
    shadowColor: COLORS.primary, shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.2, shadowRadius: 20, elevation: 8 
  },
  ...
  input: { height: 50, backgroundColor: COLORS.background, borderRadius: 16, paddingHorizontal: 15, fontSize: FONTS.sizes.md, color: COLORS.text.primary, borderWidth: 2, borderColor: COLORS.border },
  submitBtn: { backgroundColor: COLORS.primary, height: 56, borderRadius: 20, justifyContent: 'center', alignItems: 'center', marginTop: 10, shadowColor: COLORS.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 10, elevation: 5 },
